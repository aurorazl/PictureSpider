...
class WeiboSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for next_url in id_list:
            if int(next_url.split("&page=")[-1])<=max_page:
                yield Request(url="https://s.weibo.com"+next_url, callback=self.parse, headers=self.default_headers,cookies=self.cookie)
